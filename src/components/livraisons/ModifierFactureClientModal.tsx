...
export default function ModifierFactureClientModal({ facture, onClose, onSaved }: Props) {
  const [form, setForm] = useState({
    numero_facture_logiciel: facture.numero_facture_logiciel ?? '',
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError('')
    const body = {
      numero_facture_logiciel: form.numero_facture_logiciel.trim() || null,
    ...
    const res = await fetch(`/api/factures/client/${facture.id}`, {
      method: 'PATCH',
  ...
  return (
    <Modal title="Modifier la facture client" onClose={onClose} size="md">
      <form onSubmit={submit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2">
            <label className="label">N° facture</label>
            <input className="input" value={form.numero_facture_logiciel} onChange={f('numero_facture_logiciel')} placeholder="Numéro dans le logiciel..." />
          </div>
          <div>
